...
if __name__ == "__main__":
    ...
    for data_ in data_list:
        ...
        #if (data_.shape[0] > n_default_min)  and (data_.shape[0] < n_default_max) and (data_.shape[0] in to_select):
        if (data_.shape[0] > n_default_min) and (data_.shape[0] < n_default_max):

            n_cr_s = n_cr_s + 1
            hist_, bin_cent_, n_defaults_  = extract_prob_g_t(data_, perc_n)
            n_defaults2_ = data_.shape[0]

            hist_list.append(hist_)
            bin_c_list.append(bin_cent_)
            n_default_list.append(n_defaults_)
            n_default_list2.append(n_defaults2_)
    ...
    for i in range(0, len(hist_list)):
        ...
        n_def2 = n_default_list2[i]

        #label_ = ('n. def. %s'%(n_def))
        label_ = ('n. def. %s'%(n_def2))
